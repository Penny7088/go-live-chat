...
func newLanguagesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewLanguagesDao(d.DB, c.ICache.(cache.LanguagesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &languagesHandler{iDao: d.IDao.(dao.LanguagesDao)}
	iHandler := h.IHandler.(LanguagesHandler)
...
func Test_languagesHandler_Create(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := &types.CreateLanguagesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Languages))
...
func Test_languagesHandler_DeleteByID(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
...
func Test_languagesHandler_UpdateByID(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := &types.UpdateLanguagesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Languages))
...
func Test_languagesHandler_GetByID(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
...
func Test_languagesHandler_List(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListLanguagessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func Test_languagesHandler_DeleteByIDs(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteLanguagessByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = httpcli.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteLanguagessByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_languagesHandler_GetByCondition(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("GetByCondition"), &types.GetLanguagesByConditionRequest{
		query.Conditions{
...
func Test_languagesHandler_ListByIDs(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("ListByIDs"), &types.ListLanguagessByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = httpcli.Post(result, h.GetRequestURL("ListByIDs"), &types.ListLanguagessByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_languagesHandler_ListByLastID(t *testing.T) {
	h := newLanguagesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Languages)
